...
function readLabel(payload: DetectionResponsePayload | null): string | undefined {
  ...
  if (isRecord(payload) && typeof payload.label === "string" && payload.label.trim()) {
...
export default function Home() {
  ...
  const anomalyResponses = useMemo(() => {
    if (!result?.gatewayResponse?.anomaly_detection_responses) return [];
    return result.gatewayResponse.anomaly_detection_responses;
  }, [result]);
  ...
  return (
    <main className="relative overflow-hidden px-5 py-10 md:px-10">
      ...
      <section className="fade-up mx-auto flex w-full max-w-6xl flex-col gap-6">
        <header className="rounded-3xl border border-[var(--panel-border)] bg-[var(--panel)] p-6 shadow-[0_8px_40px_rgba(0,0,0,0.35)] backdrop-blur-xl">
          <p className="mono text-xs uppercase tracking-[0.3em] text-[var(--accent)]">
            MalLog Studio
          </p>
          <h1 className="mt-2 text-3xl font-semibold leading-tight md:text-4xl">
            Upload LOG and Trigger Gateway Analysis
          </h1>
          <p className="mt-3 max-w-3xl text-sm text-[var(--text-muted)] md:text-base">
            Minimal dark interface for sending a LOG upload flow and running the
            API gateway pipeline. Results are rendered as visual anomaly cards
            with key metrics, reasons, and classification color coding.
          </p>
        </header>
        ...
        <div className="flex flex-col gap-6">
          <form
            ...
            className="fade-up rounded-3xl border border-[var(--panel-border)] bg-[var(--panel)] p-6 backdrop-blur-xl"
          >
            ...
            {error ? (
              <p className="mt-3 rounded-xl border border-[rgba(255,111,97,0.45)] bg-[rgba(255,111,97,0.12)] px-4 py-3 text-sm text-[var(--danger)]">
                {error}
              </p>
            ) : null}
          </form>

          <section className="fade-up rounded-3xl border border-[var(--panel-border)] bg-[var(--panel)] p-6 backdrop-blur-xl">
